...
Se o parâmetro neste exemplo for algo como **''OR 1 = 1,** o resultado conterá todos os itens da tabela. Isso pode ser ainda mais problemático se o banco de dados suportar várias consultas e o parâmetro for **''; UPDATE USERS SET lastname = ''**. 
...
Os alertas de falha de login também se enquadram nesta categoria. Substitua mensagens como de erro como "Não encontrou esse usuário" ou "Senha incorreta" por "Falha no login" ou "Usuário/senha inválida, pois desta forma você evita de fornecer informação de que um determinado usuário existe no seu sistema. 
...
Além disso, tenha muito cuidado ao expor dados para o mundo externo. Por exemplo: se temos um endpoint em um sistema que mostra todos os nomes de usuário, não há necessidade de mostrar o id. Esse id pode ser usado para conectar outras informações mais confidenciais ao usuário, usando outros terminais. Se você usa Jackson para serializar e desserializar POJOs para JSON, tente usar **@JsonIgnore** e **@JsonIgnoreProperties** para evitar que essas propriedades sejam serializadas ou desserializadas.
...
Em 2019 houve a descoberta de uma falha de segurança no Jackson ao deserializar um JSON, conforme pode ser visto no seguinte [post](https://snyk.io/blog/jackson-deserialization-vulnerability/). Por isto é bom sempre ficarmos atentos e atualizarmos nossas dependências sempre que possível.
